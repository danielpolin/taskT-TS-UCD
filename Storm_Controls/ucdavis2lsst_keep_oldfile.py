...
def fix(infile, config_file, sensor_id, testtype, imagetype, seqnum, exptime, filter, srcpwr, mondiode, temp_a, temp_b, temp_set, stage_pos=[0.0,0.0,0.0]):

    newfile = infile.split('.fits')[0]+'_not_fixed.fits'
    ...
    mask_type = str.upper(eolib.getCfgVal(config_file, 'MASK_TYPE'))
    ...
    phdr.update('MASK_TYP', mask_type, 'Mask Type', after='FILTER')
    ...
    phdr.update('DETSIZE', '[1:4072,1:2000]', 'NOAO detector size', after='FILENAME')
    ...
    phdr.update('CCDGAIN', 'UNKNOWN', '[e/DN] estimated CCD gain', after='HEADVER')
