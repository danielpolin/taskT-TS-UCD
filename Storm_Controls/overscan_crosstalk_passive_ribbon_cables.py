...
def overscan_crosstalk_subtract(infile, newfile):

    print 'Performing overscan and crosstalk subtraction on FITS file :', infile
    ...
    for i in range(1,17) :
        ...
        for j in range(1,17):
            if i == j or ct_matrix[i-1,j-1] < 1.0E-9:
                continue
            ct_data = np.array(hdulist[j].data + 32768, dtype = np.int32)
            data = data - ct_matrix[i-1,j-1] * ct_data
