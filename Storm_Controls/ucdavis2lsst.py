...
def fix(infile, config_file, sensor_id, masktype, testtype, imagetype, seqnum, exptime, filter, srcpwr, mondiode, temp_a, temp_b, temp_set, stage_pos=[0.0,0.0,0.0]):
    ...
    if phdr['CCD_MANU'] == 'ITL' :   
        ...
        for i in range(1,17) :
            ...
            imhdr.update('LTM1_1', 1.0 - 2.0*row, after='ATV2')
            imhdr.update('LTM2_2', 1.0 - 2.0*row, after='LTM1_1')
            imhdr.update('LTV1', (col + row)*xsize * (-1 + 2 * row), after='DTV2')
            imhdr.update('LTV2', (2 * ysize) * row, after='LTV1')
    ...
    cchdr.update('TIM_FILE', eolib.getCfgVal(config_file,'TIM_FILE'), 'Timing File')
