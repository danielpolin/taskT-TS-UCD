...
zfilelist=sort(glob.glob(datadir+sys.argv[1]))
...
for i, segmentnumber in enumerate([4,5,12,13]):
    ...
    slope, intercept, r_value, p_value, std_err = stats.linregress(sort(imaxs[i,24:36]),sort(sigmaxs[i,24:36]))
    ...
    slope, intercept, r_value, p_value, std_err = stats.linregress(sort(imaxs[i,24:36]),sort(sigmays[i,24:36]))
    ...
    xlim(0.0,200000.0)
    xticks([0.0,100000,200000])
    ylim(0.90,1.10)
    xlabel('Central Peak(electrons)')
    ylabel('Sigma (Pixels)')
    legend(loc= 'lower right')
    savefig(datadir+"Forward_Model_Spots_%s_%s.png"%(firstnum,extname))
...
for i,fitsfilename in enumerate(masked_zfilelist):
    try:
        ...
        continue
slope, intercept, r_value, p_value, std_err = stats.linregress(sort(exp_current[29:39]),sort(imaxs[0,29:39]))
