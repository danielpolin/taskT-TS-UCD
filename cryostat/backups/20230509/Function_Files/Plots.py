...
def MakeTemperaturePlot(Time, Temp_A, Temp_B, filltimes, filldurations, dewartimes):
    ...
    ax1.set_xlabel("Time (Last 10 Days)")
    ...
    ax2.set_xlabel("Time (Last 10 Days)")
    ...
    ax3.plot([filltimes[0],filltimes[0]],[TAmin,TAmax],color='c',linestyle='dotted',label='Dewar Filled')

    ax3.set_xlabel("Time when cryostat filled")
    ...
    ax5.plot(filltimes[1:],timebetweenfills,'mo',label='Time Between Cryostat Fills')
    ax5.set_xlabel("Time")
    ...
    ax6.set_title("Median Time Between Fills: "+str("{:.2f}".format(numpy.median(timebetweenfills)))+'hr')
    ...
    ax7.plot(dewartimes[1:],timebetweendewar,'co',label='Time Between Dewar Fills')
    ax7.set_xlabel("Time Dewar Filled")
    ax7.set_ylabel("Time Between Dewar Fills (days)")
    ax7.legend(loc='upper left')

    #plot histogram of times between dewar fills
    ax8=fig.add_subplot(4,2,8)
    ax8.hist(timebetweendewar,color='c',edgecolor='k',linewidth=1,alpha=0.75)
    ax8.set_xlabel("Time between Dewar fills (days)")
    ax8.set_ylabel("Count")
    mediandewar=numpy.median(timebetweendewar)
    ax8.set_title("Median Time Between Dewar Fills: "+str("{:.2f}".format(mediandewar))+'days')
    timetilldewarfill=(datetime.timedelta(days=mediandewar)-(now-dewartimes[-1])).days
    if timetilldewarfill<0:
        ax7.set_title("Time To Fill the Dewar!",color='r',fontsize=20)
    else:
        ax7.set_title("Fill the Dewar in "+str(timetilldewarfill)+' days')
